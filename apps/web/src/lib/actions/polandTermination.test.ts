 ...
 * number of times, then awaited directly). Each `.from()` call pops the next
 * canned response off `queue`, in the same fixed sequence the module awaits
 * them in (employment_contracts first, then leave_ledger) — mirrors the
 * pattern already established in the leave-accrual cron's own route.test.ts.
 */
