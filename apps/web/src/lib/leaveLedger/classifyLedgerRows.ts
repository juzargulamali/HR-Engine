...
// reference_type says specifically 'opening_balance' or 'termination_settlement'
// (see isUnambiguousGrantRow below). 'deduction' and 'encashment' are
...
// 'adjustment' reference_type values that are system-generated by a single,
// specific, audited mechanism (never an arbitrary human-entered correction)
// and therefore have a deterministic, trustworthy meaning: 'opening_balance'
// (onboarding, employees.ts) and 'termination_settlement'
// (post_poland_termination_leave_adjustment(), supabase/migrations/
// 20261102000000_poland_termination_leave_true_up.sql) — the latter's signed
// amount_days is exactly the reconciling delta between the employee's real,
// prorated exit-year entitlement and what had already been granted, so
// including it (positive or negative) is required for the baseline to stay
// correct after a Poland termination, not merely "safe to include."
const DETERMINISTIC_ADJUSTMENT_REFERENCE_TYPES = new Set(["opening_balance", "termination_settlement"]);
...
function isUnambiguousGrantRow(row: AnnualLeaveLedgerRow): boolean {
  ...
  return row.entry_type === "adjustment" && row.reference_type !== null && DETERMINISTIC_ADJUSTMENT_REFERENCE_TYPES.has(row.reference_type);
}

function isAmbiguousGrantCandidateRow(row: AnnualLeaveLedgerRow): boolean {
  // Any 'adjustment' NOT tagged with one of the deterministic reference
  // types above could be a genuine opening/termination grant recorded
  // before those tags existed, or could be a wholly unrelated correction (a
  // mistake fix, a policy-transition true-up, anything) — there is no way
  // to tell which from the stored schema, so it can never safely be counted
  // as a grant, but its mere existence also means the employee's true
  // lifetime baseline can't be trusted either way.
  return row.entry_type === "adjustment" && (row.reference_type === null || !DETERMINISTIC_ADJUSTMENT_REFERENCE_TYPES.has(row.reference_type));
}
