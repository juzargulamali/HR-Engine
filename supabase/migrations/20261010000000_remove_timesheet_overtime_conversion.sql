-- Timesheets are deprecated (no nav link reaches them anymore; the routes
-- and data stay in place but the app no longer treats them as an active
-- feature). Their overtime-to-comp-day conversion was also a real bug —
-- it compared the WHOLE timesheet period's hours against a "weekly"
-- threshold, so a monthly timesheet could wildly over-credit comp days
-- for normal, non-overtime work. Attendance already has its own correct
-- comp-day mechanism (bulkRecordAttendance in
-- apps/web/src/lib/actions/attendance.ts: marking someone "present" on a
-- flagged weekend/holiday credits one comp day, keyed to that attendance
-- record so re-saving never double-credits) — that's the one path going
-- forward. This just removes the superseded, buggy path; approving a
-- timesheet still marks it approved, it just no longer converts anything.
