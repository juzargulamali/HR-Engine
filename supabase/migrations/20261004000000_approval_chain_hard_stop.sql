-- decide_leave_approval()'s step-walking loop treated "no one holds the
-- required role" exactly like "the resolved approver is the requester
-- themselves" — both just fell through to try the next step, and if no
-- step ever produced a different eligible approver, the whole entity
-- silently finalized as fully approved. For a workflow whose later step is
-- a role nobody currently holds (e.g. no CEO assigned in this company right
-- now), that means the request gets marked approved without that mandatory
-- reviewer ever actually seeing it — a real control bypass, not just a
-- UX gap, most consequential for payroll_export_run's "Finance then CEO,
-- always" contract (see its own comment further down this schema).
--
-- Fix: distinguish the two cases. Self-approval still skips forward to try
-- the next step (unchanged, already covered by existing tests). A step
-- with literally no eligible approver now aborts the whole decision with a
-- clear, actionable error instead of silently continuing — the decision
-- that triggered this call rolls back too (it's all one transaction), so
-- the approver can simply retry once HR Admin has assigned the missing
-- role.
...
begin
  ...
  -- (self-approval prevention). A step that resolves to NO ONE AT ALL
  -- (the role has zero holders in this company) is different: that's not
  -- "skip and keep going", it's "this cannot legitimately proceed" — abort
  -- the whole decision rather than silently finalizing as if this step had
  -- been satisfied.
  for v_step in
